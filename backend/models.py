...
class Receta(db.Model):
    ...
    rendimiento_vasos = db.Column(db.Integer, nullable=False)   # aguas que rinde una llenadora
    volumen_jarra = db.Column(db.Float)                          # litros (informativo)
    activa = db.Column(db.Boolean, default=True)
    insumos = db.relationship("RecetaInsumo", backref="receta",
...
class Insumo(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    nombre = db.Column(db.String, nullable=False)
    unidad = db.Column(db.String, default="kg")                 # todo se captura por kilo
    precios = db.relationship("PrecioInsumo", backref="insumo",
                              cascade="all, delete-orphan")
...
class Produccion(db.Model):
    ...
    receta_id = db.Column(db.Integer, db.ForeignKey("recetas.id"), nullable=False)
    vasos = db.Column(db.Integer, nullable=False)               # llenadoras producidas
    ...
    detalles = db.relationship("ProduccionDetalle", backref="produccion",
                               cascade="all, delete-orphan")
